...
data.each_with_index do |char_data, i|
  ...
  if char_data.key?("deepseek_description_pl_paragraph_1") && !char_data["deepseek_description_pl_paragraph_1"].to_s.strip.empty?
    puts "  пропуск — уже есть deepseek_description_pl_paragraph_1"
    next
  end

  begin
    result = generate_for_deepseek(char_data)
